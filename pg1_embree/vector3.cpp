...
Vector3 Vector3::ReflectTo(const Vector3& u) const
{
	Vector3 reflected = *this - 2 * (this->DotProduct(u)) * u;
	reflected.Normalize();
	return reflected;
}

Vector3 Vector3::ReflectFrom(const Vector3& u) const
{
	Vector3 reflected = 2 * (this->DotProduct(u)) * u - *this;
	reflected.Normalize();
	return reflected;
}

Vector3 Vector3::FromDirectionTo(const Vector3& u) const
{
	Vector3 direction = u - *this;
	direction.Normalize();
	return direction;
}
